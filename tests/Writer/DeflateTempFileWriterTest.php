...
final class DeflateTempFileWriterTest extends TestCase
{
    ...
    public function testInvalidCompressionEncoding(int $encoding): void
    {
        ...
        new DeflateTempFileWriter($encoding);
    }
    ...
    public function testInvalidCompressionLevel(int $level): void
    {
        ...
        new DeflateTempFileWriter(ZLIB_ENCODING_GZIP, $level);
    }
    ...
    public function testInvalidCompressionMemory(int $memory): void
    {
        ...
        new DeflateTempFileWriter(ZLIB_ENCODING_GZIP, -1, $memory);
    }
    ...
    public function testInvalidCompressionWindow(int $window): void
    {
        ...
        new DeflateTempFileWriter(ZLIB_ENCODING_GZIP, -1, 9, $window);
    }
    ...
    {
        $this->writer = new DeflateTempFileWriter($encoding, $level, $memory, $window);
        $this->writer->start($this->filename);
        $this->writer->append('foo');
    ...
    public function testBrokenWindow(int $level, int $memory): void
    {
        ...
        $this->writer = new DeflateTempFileWriter(ZLIB_ENCODING_DEFLATE, $level, $memory, $expected_window);
        $this->writer->start($this->filename);
        $this->writer->append('foo');
        $this->writer->append('bar');
        $this->writer->finish();
